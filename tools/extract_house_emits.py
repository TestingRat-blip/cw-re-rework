...
# The nine arms in the order the port indexes them:  village desc[+0x1c] 0..5, then the
# three ruin arms.  `0x4e5c6a`-`0x4e5c77` dispatches the village half on
# `(unsigned)(d1c - 1) <= 4`, so d1c 1..5 take a jump-table arm and everything else the
# default; `0x4e6202`-`0x4e6218` dispatches the ruin half on `d1c == 2`, `== 3`, else.
CPP_ORDER = ["village d1c=0", "village d1c=1", "village d1c=2", "village d1c=3",
             "village d1c=4", "village d1c=5", "ruin d1c=2", "ruin d1c=3", "ruin other"]
GENERIC_ENTRANCE = 0x889          # models[0x889], loaded at 0x4e5c42 before the dispatch
CARPETS = [0x88A, 0x88B, 0x88C]   # models[0x88a + rand() % 3] at 0x4ea28d


def face_shifts(data, secs, md):
    """The four ENTRANCE-STAIRS emits' axis constants, by the face walk's orient literal.

    Every other emit adds `+7` on both axes -- the module centre RE_town_furnish.md 5b.2
    derives independently.  The entrance emits add -6 or +20 on the face's own axis, i.e. a
    whole 13-block module onto the neighbour the face looks at.  Both halves are asserted
    here so the header cannot be generated from a binary that says something else.
    """
    out, others = {}, 0
    for e in describe(data, secs, md):
        if e["slot"] is None:
            continue
        cx, cz = e["y"].c, e["z"].c        # e["y"] is vec3's first argument = game X
        if MODEL_SLOT.get(e["slot"]) == "ENTR":
            orient = e["orient"].c if e["orient"] is not None else None
            if orient not in (0, 1, 2, 3):
                raise SystemExit("entrance emit 0x%x has orient %s" % (e["emit"], orient))
            if sorted((cx, cz)) not in ([-6, 7], [7, 20]):
                raise SystemExit("entrance emit 0x%x adds (%d, %d)" % (e["emit"], cx, cz))
            out[orient] = (cx, cz, e["x"].c)
        else:
            if (cx, cz) != (7, 7):
                raise SystemExit("emit 0x%x adds (%d, %d), expected (7, 7)"
                                 % (e["emit"], cx, cz))
            others += 1
    if sorted(out) != [0, 1, 2, 3]:
        raise SystemExit("the entrance emits cover orients %s" % sorted(out))
    dy = {v[2] for v in out.values()}
    if dy != {-4}:
        raise SystemExit("the entrance emits' Y offsets are %s" % dy)
    return out, others


def emit_cpp(data, secs, md):
    sets = model_sets(data, secs, md)
    db = json.load(open(MODEL_MAP, encoding="utf-8"))["world_model_db"]
    shifts, others = face_shifts(data, secs, md)

    def dims(mid):
        d = db[str(mid)]["dims"]
        if not all(0 <= v <= 255 for v in d):
            raise SystemExit("model 0x%x has dims %s" % (mid, d))
        return d

    print("// GENERATED by tools/extract_house_emits.py --cpp in cw_decomp -- DO NOT EDIT.")
    print("// It INTERPRETS the HOUSE ENTITY pass out of Server.exe (the nine model-set arms")
    print("// at 0x4e5c39-0x4e6402 and the 33 emit blocks of 0x4e74a5-0x4ea988) and joins the")
    print("// ids to cw_rederive/model_id_map.json for the .cub dimensions.")
    print("// gate_town_entities.py re-runs the generator every time and diffs the result")
    print("// against these literals, so a hand edit fails the gate (lesson 7c/7i).")
    print("//")
    print("// %d emits add (+7, +7) on both axes; the four ENTRANCE-STAIRS emits add a whole"
          % others)
    print("// module onto their own face's neighbour, which is the table at the bottom.")
    print("#pragma once")
    print("#include <cstdint>")
    print("")
    print("namespace vx::cw {")
    print("")
    print("// One arm of the model-set dispatch: thirteen consecutive ids in the roles")
    print("//   base, floor, floor-stairs, wall, wall-window, wall-door, wall-indoor,")
    print("//   wall-balcony, wall-lamp, roof1, roof2, roof3, arc")
    print("// plus the entrance-stairs model, which is models[0x889] unless the arm")
    print("// overwrites it. `dim` is the .cub width/depth/height of each, entrance last.")
    print("struct TownEntitySet { int16_t id[14]; uint8_t dim[14][3]; };")
    print("")
    print("// index: a VILLAGE takes desc[+0x1c] when it is 1..5 and arm 0 otherwise;")
    print("// a RUIN takes 6 for desc[+0x1c] == 2, 7 for == 3 and 8 otherwise.")
    print("constexpr TownEntitySet kTownEntitySets[9] = {")
    for name in CPP_ORDER:
        s = sets[name]
        ids = list(s["base"]) + [s["entr"] or GENERIC_ENTRANCE]
        print("    {  // %s -- %s" % (name, db[str(ids[0])]["name"].split("-")[0]))
        print("        {%s}," % ", ".join("0x%03x" % i for i in ids))
        print("        {%s}," % ", ".join("{%d,%d,%d}" % tuple(dims(i)) for i in ids))
        print("    },")
    print("};")
    print("")
    print("// The wall/roof walk's carpets: models[0x88a + rand() % 3] at 0x4ea28d.")
    print("constexpr int16_t kTownEntityCarpetId[3] = {%s};"
          % ", ".join("0x%03x" % i for i in CARPETS))
    print("constexpr uint8_t kTownEntityCarpetDim[3][3] = {%s};"
          % ", ".join("{%d,%d,%d}" % tuple(dims(i)) for i in CARPETS))
    print("")
    print("// Per FACE WALK (indexed by the walk's own orient literal 0 -Z, 1 -X, 2 +Z,")
    print("// 3 +X): the entrance-stairs emit's X and Z constants, where every other emit")
    print("// uses +7. Its Y offset is a flat -4 at all four.")
    print("constexpr int8_t kTownEntityEntranceShift[4][2] = {%s};"
          % ", ".join("{%d,%d}" % (shifts[o][0], shifts[o][1]) for o in range(4)))
    print("constexpr int32_t kTownEntityEntranceDy = %d;" % shifts[0][2])
    print("")
    print("}  // namespace vx::cw")
    return 0
