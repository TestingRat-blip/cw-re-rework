...
7aa. **A gate that checks the claim against the SAME bytes the claim was read off cannot
   falsify it — and "this input is not available" is a claim about which MAP you looked
   in.** Both halves are the 07-29g slice. `RE_town_entities.md` §3.1 read each model
   arm's base off its **first `push imm32`**, and `gate_town_entities.py` then asserted
   the base by re-reading that same first push: 46,338 green checks over a table that is
   wrong for five of the nine arms, because clay, whiteclay and all three ruin arms open
   by overwriting the *entrance-stairs* slot and only then load their thirteen. A check
   is only a check if it comes from somewhere else — here, interpreting the STORES.
   ★ The other half is worth more. §9 had blocked the port on "a per-emit-site model
   anchor that lives in the server's model DB and nothing in `cwgen` reads", and gave a
   *correct* reason for not naming the models: `prop_ids.json` is the `site+0xc`
   namespace and these ids go elsewhere (lesson 7t, applied honestly). The elsewhere has
   a map too — `model_id_map.json`, 2,550 models with their `.cub` dimensions, which
   `RE_town_antique.md` had used for the same `m->[+0x44]` field **one day earlier**.
   Feeding it in predicted 19,352 of 19,352 record positions. **When a decode records
...
**What is left is the PORT.** Thirteen of the builder's stages are now in `cwgen`; **one is
RE'd and gated with no port at all**, and it is the one whose blocker is not in the builder
at all.
...
✅ **Item 5 is DONE (07-29f)** — `rederive_townnpcs` 231/231, and it kept its promise from
...
1. ✅ **The rest of the town chain — the RE half is DONE (07-29c), 176/176 sites, 100.00%.**
   ...
   **176 of 176 firing rand sites, 228,413 of 228,413 recorded draws — 100.00%, nothing
   ...
   ▶ **So the town work that is LEFT is the PORT** — and as of 07-29f it is one stage from
   done. The MARKET, the ANTIQUE BUILDINGS, the HOUSE SURROUND pass, the RUIN OCCUPANT pass
   and the VILLAGE NPC pass are **in cwgen and gated ab initio**; **one stage** is still
   RE'd and gated with no port: the HOUSE ENTITY pass (`RE_town_entities.md` §9), whose
   blocker is the server's model DB and is not in the builder at all.
   ★ **The blocker the two NPC stages shared is gone** — `site+0x88` is derived
   (`RE_town_buildings.md`, `gate_town_buildings` 25/25) — and 07-29f closed the rest by
   deriving the five per-town bits. Each of the five that landed confirmed its FED table was
   accurate, and four of the five were optimistic in the right direction: the market needed
   no terrain, the surround needed only a base Y, the ruin pass turned out not to need the
   vector sizes its own §9 asked for, and the NPC pass needed exactly one terrain read where
   §8 listed five open inputs.
...
three make it **59 / 27**, 07-29e's one **60 / 27**, and 07-29f's one
**61 `[PASS]` / 27 `[skip]`**. (The isolation runs confirm the
