...
*Status:* **RE + gate only. NOT ported** — §9 says what a port would have to be fed.
...
| the building list `site+0x88` and each `building+0x24` (**sizes only**) | **not derived** — same gap as `RE_town_npcs.md` §8; the pick is `rand() % size`, so only the SIZE advances the stream |
